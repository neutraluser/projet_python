...
background = pygame.image.load("personnage/background.jpg")  # Remplace par ton image
background = pygame.transform.scale(background, (WIDTH, HEIGHT))
# Afficher le texte "Sélectionner un personnage pour le joueur 1"


# Chargement des images
perso_feu = pygame.image.load("personnage/feu.png")
perso_eau = pygame.image.load("personnage/eau.png")
perso_foudre = pygame.image.load("personnage/foudre.png")
...
def afficher_ecran_selection(player):
    """
    ...
    # Afficher le texte "Sélectionner un personnage pour le joueur 1"
    font = pygame.font.Font(None, 70)  # Taille de la police augmentée pour le titre
    texte = font.render(f"Sélectionner un personnage pour le {player}", True, WHITE)
    texte_x = (WIDTH - texte.get_width()) // 2
    texte_y = 70  # Position en haut de l'écran
...
def selectionner_personnage(player):
    running = True
    personnage_choisi = None
    while running:
        afficher_ecran_selection(player)
...
if __name__ == "__main__":
    selectionner_personnage("jouer 2")
